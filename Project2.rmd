...
We will perform a t-test on these data as the size of the data is quite small (60 total observations, 10 guinea pigs per supplement). We will perform a t-test comparing delivery method at each dosage level. 
...
```{r}
#10 guinea pigs per supplement (10-1=9, 95% confidence interval)
#For what t-test value should we accept the null hypothesis? t value must be greater than:
qt(.975,9)
```
...
After this test we accept the null hypothesis as the difference is not equal to zero, it appears that OJ promotes larger tooth growth at 0.5mg. We could infer this with the initial chart. 
...
After this test we accept the null hypothesis as the difference is not equal to zero, it appears that OJ promotes larger tooth growth at 1mg. We could infer this with the initial chart. 
...
After this test we reject the null hypothesis as the difference could be zero at 2mg dosage level. We could infer this with the initial chart. 
